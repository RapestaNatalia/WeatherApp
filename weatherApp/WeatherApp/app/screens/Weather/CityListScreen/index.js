...
import {getLocations,getCurrentWeatherPosition,getForecastWeatherPosition} from 'actions/weather';
...
const CityListScreen= ({navigation})=>{
    ...
    const {currentWeather,forestWeather} = useSelector(state => state.weatherReducer);
    ...
    useEffect(() => {
          ...
          new Promise(function() {
              ...
              let result = values.map(localities => ({ id: localities.id, name: localities.name }));
              setLocalities(
      ...
      return (
        ...
        />}
          {!isLoading && localities && <FlatList data={localities} renderItem={renderItem} />}
        </Container>
      );
