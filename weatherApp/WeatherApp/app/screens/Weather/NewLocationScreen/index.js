...
import {Container,IconContainer,RightButtons,ButtonContainer,WeatherImageContainer,FlatListContainer, Item,LocalityOption,TempContainer, CityContainer} from './styled';
    ...
    const renderItem = ({item ,index}) => {
      ...
      if (index ===0) {
        ...
        const rightButton=[{
          ...
          disabled:greeting,
          onPress:() =>{dispatch(deleteACity(item.id))}
        }]
        ...
        return (
         ...
         {item && <LocalityOption
          ...
          key={index.toString()}
          >
           {item.temp &&<TempContainer>  
           <Item>{item.temp} ºC</Item></TempContainer> }
           {item.city && <CityContainer>
           <Item>{item.city}</Item></CityContainer>}
           {item.icon && <WeatherImageContainer>
               <Icon
